...
int al_add(ArrayList* pList,void* pElement)
{
    ...
    if(pList!=NULL && pElement!=NULL){
        ...
        if(pList->size < pList->reservedSize && returnAux==0){
...
ArrayList* al_clone(ArrayList* pList)
{
    ...
    if(pList!=NULL){
        ...
        for(i=0;i<=pList->size;i++){
            lista->pElements[i]=pList->pElements[i];
        }
        lista->size=pList->size;
        returnAux=lista;
        }
    return returnAux;
}
...
ArrayList* al_subList(ArrayList* pList,int from,int to){
    int i=0;
    ArrayList* returnAux = NULL;
    if(pList!=NULL && from>=0 && from<=to && to>=0){
        ArrayList* lista=al_newArrayList();
        for(i=from;i<=to;i++){
            lista->pElements[i]=pList->pElements[i];
        }
    ...
    if(pList!=NULL && pFunc!=NULL && order>=0 && order<=1 ){
        while(flag){
            ...
            for(i=0;i<pList->size-1;i++){
                if(order){
                    if(pFunc(pList->pElements[i+1], pList->pElements[i])==-1){
                        aux->pElements[i]=pList->pElements[i+1];
                        pList->pElements[i+1]=pList->pElements[i];
                        pList->pElements[i]=aux->pElements[i];
                        flag=1;
                    }
                }
            else{
                if(pFunc(pList->pElements[i+1], pList->pElements[i])==1){
                    ...
                    pList->pElements[i+1]=aux->pElements[i];
                    flag=1;
                    }
                }
            }
 ...
 *                  - ( 0) if Ok
 */
int contract(ArrayList* pList,int index)//SE LO PUSE AL POP Y AL REMOVE
{
    int i;
    int returnAux = -1;
    if(pList!=NULL && index>=0 && index<=pList->size){
